...
class BoostDeserializer extends StdDeserializer<Boost> {
    ...
    public Boost deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        ...
        if (typeNode instanceof TextNode) {
            ...
            if (Boost.BoostType.PROXIMITY.equals(boostType)) {
                return deserializeProximityBoost(node,jsonParser);
                ...
                return deserializeValueBoost(node);
            }
        }
        return null;
    }

    private Boost deserializeProximityBoost(final TreeNode node, final JsonParser jsonParser) throws JsonProcessingException {
    ...
    private Boost deserializeValueBoost(final TreeNode node){
